...
const ChooseCorrectClass: React.FC<ChooseCorrectClassProps> = ({
  uploadedFileName,
  onButtonClick,
}) => {
  ...
  const correctDiseaseClass = async (
    imageName: string,
  ...
  const handleClassButtonClick = (className: string) => {
    ...
    setShowBoundingBoxButton(false)
  }
      ...
      <div className="button-container">
        <Grid gap={2}>
          {Object.keys(classes).map((className) => (
            <div
              key={className}
              className={`class-button-wrapper ${
